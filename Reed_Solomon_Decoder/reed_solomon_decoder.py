...
def rs_find_error_locator(synd, nsym, erase_loc=None, erase_count=0):
    ...
    if (errs-erase_count) * 2 + erase_count > nsym:
        print("We couldn't correct everything")    # too many errors to correct
...
def rs_correct_msg(msg_in, nsym, erase_pos=None):
    ...
    return (msg_out[:-nsym], msg_out[-nsym:],len(err_pos)) # also return the corrected ecc block so that the user can check()

init_tables()
# prim = 0x11d
# n = 20 # set the size you want, it must be > k, the remaining n-k symbols will be the ECC code (more is better)
# k = 10 # k = len(message)
# message = [ 1, 12, 2, 4, 5, 6, 67, 8, 9, 0xa,0xe5,0xfa,5,0xc4,0xf3,0x1e,0x26,0x46,77,9]
# corrected_message, corrected_ecc = rs_correct_msg(message, n-k)
